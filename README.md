...
- **Step1. 将原问题归结为其等价问题** 
    ...
    引进新的变量``y(t)=x'(t)``将**高阶微分方程的初值问题**归结为如下**一阶微分方程组的初值问题**来求解.
    ```math
    \begin{equation}
    \left\{
    \begin{aligned}
    x'(t) & = y, &  t\in [1,5]\\
    y'(t) & = \frac{t^3\ln t +2ty -2y}{t^2}, &  t\in [1,5]\\
    x(t) & = y, &  t=1\\
    y(t) & = 0. &  t=1
    \end{aligned}
    \right.
...
- **Step2. 四阶龙格-库塔方法的离散格式** 
    ...
    \end{equation}
    ```
    这是一步法，利用节点``t_n``上的值``x_n,y_n``，由 (4) 式顺序计算 ``K_1,L_1,K_2,L_2,K_3,L_3,K_4,L_4``，然后代入(3)式即可求得节点``t_{n+1}``上的 ``x_{n+1},y_{n+1}``.
    - **Step3. 利用龙格-库塔法求解高阶微分方程的```Python```代码如下:**
    ```Python
    # 开发者:    Leo 刘
    # 开发环境: macOs Big Sur
    ...
    y:		 0.0000		 0.0991		 0.4551		 1.1731		 2.3552		 4.0981		 6.4929		 9.6255		 13.5777		 18.4265		 24.2458
     -------------------------------------------------------------------------------------------------------------------------------------------------
    ```
    ![龙格-库塔_数值结果](media/16328493476504/%E9%BE%99%E6%A0%BC-%E5%BA%93%E5%A1%94_%E6%95%B0%E5%80%BC%E7%BB%93%E6%9E%9C.png)
